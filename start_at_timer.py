...
def start_timer(user_id,subscription_period):
    if str(user_id) == admin_id:
        ...
        os.system(f"echo 'python3 {del_script_sub} {user_id}' | at now +{subscription_period} minute")
    else:
        print('Старт таймера')
        os.system(f"echo 'python3 {del_script_sub} {user_id}' | at now +{subscription_period} days")
