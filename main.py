...
def handle_paid_key(callback):
    ...
    if outline.user_key_info(user_key_id):
        ...
        bot.send_message(callback.message.chat.id,f'У вас уже имеется ключ, проерить его /mykeys')
...
def check_payment_status(callback):
    ...
    payment_status = invoice_management.payment_verification(test_libel)
